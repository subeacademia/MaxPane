-- @description ReDockIT - Apply Saved Layout 1 (Native Docking)
-- @author ReDockIT
-- @version 2.0.0
-- @about Quick-apply script for ReDockIT saved layout slot 1. Bind to a shortcut.
...
local function load_layout(slot)
  return deserialize(reaper.GetExtState(EXT_SECTION, "layout_" .. slot))
end
...
-- Configure dockers and assign windows
for _, zone in ipairs(layout.zones) do
  -- Set docker position
  if HAS_SWS then
  ...
  if size_var and HAS_SWS then
    reaper.SNM_SetIntConfigVar(size_var, zone.size)
  end
  ...
  for _, wname in ipairs(zone.windows) do
    ...
    if ident then
      reaper.Dock_UpdateDockID(ident, zone.docker)
    end
  end
end

-- Reopen windows with deferred execution
local open_list = {}
...
local function deferred_open()
  if open_idx > #open_list then
    ...
    reaper.ShowConsoleMsg("[ReDockIT] Layout 1 applied (native docking).\n")
    return
  end
